...
function buildCategories() {
    ...
    [...allClasses].sort().forEach(cls => {
        ...
        if (grade >= 1 && grade <= 6) classGroups[gradeNames[grade]].push(cls);
    });
    ['一年級','二年級','三年級','四年級','五年級','六年級'].forEach(g => {
...
function populateQueryUI() {
    populateGradeSelect('sel1', classGroups['一年級']);
    populateGradeSelect('sel2', classGroups['二年級']);
    populateGradeSelect('sel3', classGroups['三年級']);
    populateGradeSelect('sel4', classGroups['四年級']);
    populateGradeSelect('sel5', classGroups['五年級']);
    populateGradeSelect('sel6', classGroups['六年級']);
